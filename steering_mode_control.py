...
class NMPCSimulatorSystem():
    ...
    def calc_predict_and_adjoint_state(self, x, y, yaw, v, u_1s, u_2s, N, dt):
        x_s, y_s, yaw_s, v_s = self._calc_predict_states(x, y, yaw, v, u_1s, u_2s, N, dt)  #State Equation
        
        lam_1s, lam_2s, lam_3s, lam_4s = self._calc_adjoint_states(x_s, y_s, yaw_s, v_s, u_1s, u_2s, N, dt) #Adjoint Equation
    ...
    def _calc_predict_states(self, x, y, yaw, v, u_1s, u_2s, N, dt):
        ...
        for i in range(N):
            temp_x_1, temp_x_2, temp_x_3, temp_x_4 = self._predict_state_with_euler(x_s[i], y_s[i], yaw_s[i], v_s[i], u_1s[i], u_2s[i], dt)
            x_s.append(temp_x_1)
            y_s.append(temp_x_2)
    ...
    def _calc_adjoint_states(self, x_s, y_s, yaw_s, v_s, u_1s, u_2s, N, dt):
        ...
        for i in range(N -1, 0, -1):
            temp_lam_one, temp_lam_two, temp_lam_three, temp_lam_four = self._adjoint_state_with_euler(x_s[i], y_s[i], yaw_s[i], v_s[i], lam_1s[0], lam_2s[0], lam_3s[0], lam_4s[0] ,u_1s[i], u_2s[i], dt)

            lam_1s.insert(0, temp_lam_one)
            lam_2s.insert(0, temp_lam_two)
            lam_3s.insert(0, temp_lam_three)
            lam_4s.insert(0, temp_lam_four)

        return lam_1s, lam_2s, lam_3s, lam_4s


    def _predict_state_with_euler(self, x, y, yaw, v, u_1, u_2, dt):

        dx, dy, dyaw, dv = differential_model(v, yaw, u_1, u_2)

        next_one = x + dt * dx
        next_two = y + dt * dy
        next_three = yaw + dt * dyaw
        next_four = v + dt * dv

        return next_one, next_two, next_three, next_four

    def _adjoint_state_with_euler(self, x, y, yaw, v, lam_1, lam_2, lam_3, lam_4, u_1, u_2, dt):

        prem_lam_1 = lam_1 + dt * 0.0
        prem_lam_2 = lam_2 + dt * 0.0
        pre_lam_3 = lam_3 + dt * (- lam_1 * math.sin(yaw) * v + lam_2 * math.cos(yaw) * v)
        pre_lam_4 = lam_4 + dt * (lam_1 * math.cos(yaw) + lam_2 * math.sin(yaw) + lam_3 * math.sin(u_2) / WB)

        return prem_lam_1, prem_lam_2, pre_lam_3, pre_lam_4 
...
class NMPCController_with_CGMRES():
    ...
    def calc_input(self, x, y, yaw, v, time):
        ...
        # Fxt:F(U,x+hx˙,t+h)
        Fxt = self._calc_f(x_s, y_s, yaw_s, v_s, lam_1s, lam_2s, lam_3s, lam_4s, self.u_1s, self.u_2s, self.dummy_u_1s, self.dummy_u_2s, self.raw_1s, self.raw_2s, self.N, dt)
        ...
        F = self._calc_f(x_s, y_s, yaw_s, v_s, lam_1s, lam_2s, lam_3s, lam_4s, self.u_1s, self.u_2s, self.dummy_u_1s, self.dummy_u_2s, self.raw_1s, self.raw_2s, self.N, dt)
        ...
        Fuxt = self._calc_f(x_s, y_s, yaw_s, v_s, lam_1s, lam_2s, lam_3s, lam_4s, self.u_1s + du_1, self.u_2s + du_2, self.dummy_u_1s + ddummy_u_1, self.dummy_u_2s + ddummy_u_2, self.raw_1s + draw_1, self.raw_2s + draw_2, self.N, dt)
    ...
    def _calc_f(self, x_s, y_s, yaw_s, v_s, lam_1s, lam_2s, lam_3s, lam_4s, u_1s, u_2s, dummy_u_1s, dummy_u_2s, raw_1s, raw_2s, N, dt):

        F = []
        for i in range(N):
            # ∂H/∂u(xi, ui, λi)
            F.append(u_1s[i] + lam_4s[i] + 2.0 * raw_1s[i] * u_1s[i])
            F.append(u_2s[i] + lam_3s[i] * v_s[i] / WB * math.cos(u_2s[i])**2 + 2.0 * raw_2s[i] * u_2s[i])
...
def plot_figures(plant_system, controller, iteration_num, dt):
    ...
    fig_traj.plot(plant_system.history_x, plant_system.history_y, "-r")
...
def plot_car(x, y, yaw, steer = 0.0, cabcolor="-r", truckcolor="-k"):
    ...
    #fr_wheel == Front Wheel (RIGHT) and fl_wheel = Front Wheel (LEFT)
    # rl_wheel = Rear Wheel (LEFT) and rr_wheel = Rear Wheel (RIGHT)
...
def animation(plant, controller, dt):
    ...
    for t in range(1, len(controller.history_u_1), skip):
        ...
        plt.axis('equal')
        plt.grid(True)
        plt.title("Time[s]:" + str(round(time, 2)) + ",acceleration[m/s]" + str(round(accel, 2)) + ", speed[km/h]:" + str(round(v * 3.6, 2)))
        plt.pause(0.0001)
